...
class XPathTag extends XPathExpr {
	...
	Vector<Node> eval() {
		...
		//Envoyer tous les noeuds du documents qui sont des textes	
		case "text()":
			res = new Vector<>();
			for(Node n : this.dom) {
	  ...
	  //Renvoyer tous les noeuds qui sont des elements mais pas autres choses		
		case "*":
			res = new Vector<>();
			for(Node n : this.dom) {
				//verifier que le type du noeud c'est un element
